...
namespace Assets.Korzina.Scripts
{
    public class GirlSwitcher : MonoBehaviour
    {
        ...
        public static void OnIndexChange(int index)
        {
            ...
            var prev_tier_obj = new TierObject(index - 1);
            if (prev_tier_obj.tier < player_manager.current_tier && index > 0)
                _instance.left.interactable = false;
            else
                _instance.left.interactable =;
            var tier_obj = new TierObject(index+1 );

            //if next index tier bigger than our current or
            //that tier equal max achieved and bigger than max achieved girl or pos
            if(tier_obj.tier>player_manager.current_tier
