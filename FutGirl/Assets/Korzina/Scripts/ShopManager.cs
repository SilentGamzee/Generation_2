    ...
    public class ShopManager : MonoBehaviour
    {
           ...
           // SaveInfo.ClearSave();
            ...
            for (var i = 0; i < PlayerManager._instance.Tiers_count; i++)
            {
                ...
                if (i > 0)
                {
                    ...
                    upgradeGirlIndexes.Add(butt_b, TierObject.GetIndexByTier(i ));
                    cost = (int)(cost * cost_mult);
                }
                else
                {
                    if (!SaveInfo.upgrades.Contains(0))
                        SaveInfo.upgrades.Add(0);
                    upgradeDict.Add(butt_b, 0);
                    upgradeGirlIndexes.Add(butt_b, 0);
            ...
            foreach (var i in SaveInfo.upgrades)
            {
            ...
            adsExitButton.onClick.AddListener(OnAdsClose);
        }
        ...
        public static void OnMoneyChanged(int money)
        {
            ...
            foreach (var kv in upgradeDict)
            {
                var butt = kv.Key;
                var cost = kv.Value;
                if (money >= cost || SaveInfo.upgrades.Contains(upgradeDict[butt]))
