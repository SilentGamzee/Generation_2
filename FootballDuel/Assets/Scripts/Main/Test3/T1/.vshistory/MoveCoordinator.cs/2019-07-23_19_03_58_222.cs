...
namespace Assets.Scripts.Main
{
    class MoveCoordinator : MonoBehaviour
    {
        ...
        void DoArtificalMove()
        {
            ...
            var hero_pos = ButtonListener._instance.myhero.transform.position;
            var r = UnityEngine.Random.Range(-10, 10);
            hero_pos.y += r;
            Vector3 diff = hero_pos - enemy.transform.position;
            diff.Normalize();

            var item = Instantiate(ButtonListener._instance.ball, enemy.transform);
            item.AddComponent<BallCollider>();
            var rb2D = item.GetComponent<Rigidbody2D>();
            rb2D.AddForce(diff * 0.07f);
        }
