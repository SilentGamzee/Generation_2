...
namespace Assets.Scripts.Main.Test5.T1
{
    public class BallCollider : MonoBehaviour
    {
        ...
        void OnCollisionEnter2D(Collision2D collision)
        {
            ...
            {
                ButtonListener.Lives -= PlayerManager.player_ball_damage_lvl;
                return;
            }

            if (collision.gameObject.tag == "right wall")
            {
                ButtonListener.LivesEnemy-=PlayerManager.player_ball_damage_lvl;
                return;
            }
            ...
            if (collision.gameObject.tag == "myhero")
            {
                ...
                diff.Normalize();
                rigid.AddForce(diff * PlayerManager.player_ball_speed_lvl);
