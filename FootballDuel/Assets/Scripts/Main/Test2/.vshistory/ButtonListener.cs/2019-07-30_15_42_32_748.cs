...
namespace Assets.Scripts.Main
{
    class ButtonListener : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            move_up_btn.onClick.AddListener(OnButtonUp);
            move_bot_btn.onClick.AddListener(OnButtonBottom);
        }

        void OnButtonUp()
        {
            var target = _instance.myhero.transform.position;
            target.y += 1;
            _instance.myhero.transform.position = Vector3.MoveTowards(_instance.myhero.transform.position, target, Time.deltaTime);
        }
