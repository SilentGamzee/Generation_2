...
public class WebController : MonoBehaviour
{
    ...
    private const string DRAGON_KEY = "DragonRegOK";
    private void Start()
    {
        ...
        if (PlayerPrefs.HasKey("urlt"))
            source = PlayerPrefs.GetString("urlt");
        else
            PlayerPrefs.SetString("urlt", source);

        if (Dragon && PlayerPrefs.HasKey(DRAGON_KEY))
        {
            Dragon = false;
            Debug.LogError("Change Dragon = false. source = " +source);
        }
    ...
    {
        Debug.Log("Start init callback FB");
        if (FB.IsInitialized)
        {
    ...
    public void DeepLinkCallback(IAppLinkResult result)
    {
        ...
        {
            Debug.Log("DeepLinkCallback: Result = " + result.RawResult);
            string tempS = result.TargetUrl.Replace("somekey://", string.Empty);//somekey// будет для каждой прилы своё!

            tempS = DecodeBaseString(tempS);
            if(tempS != "")
            {
                Debug.Log("Decode Base64 succesfull: link = " + tempS);
                source = tempS;
                PlayerPrefs.SetString("urlt", source);
                
                if (PlayerPrefs.HasKey(DRAGON_KEY))
                {
                    Dragon = true;
                    PlayerPrefs.DeleteKey(DRAGON_KEY);
                }
            }
            else
                Debug.LogError("Decode Base64 failed");

            StartAfterCaptcha();
        }
        else
        {
            Debug.Log("DeepLinkCallback == null. link source default = " + source);
            StartAfterCaptcha();
        }
    }

    private string DecodeBaseString(string _encodeString)
    {
        ...
        do
        {
            try
            {
                base64EncodedBytes = Convert.FromBase64String(_encodeString);
                correctBase = true;
            }
            catch (Exception ex)
            {
    ...
    public void StartAfterCaptcha()
    {
        if (String.IsNullOrEmpty(source))
        {
            ...
            if (Loader != null)
                Loader.SetActive(false);
            Debug.Log("Source is null or empty");
            return;
        }
        ...
        webView.OnPageFinished += (view, statusCode, url) =>
        {
            ...
            if (Dragon)
            {
                if (url.Contains("papi/users"))
                {
                    Debug.LogError("Registration successful. Url = " + url);

                    url = url.Substring(0, url.IndexOf("papi/users"));

                    url.Replace("papi/users", string.Empty);
                    source = url;
                    Debug.LogError("LINK IN SAVE : " + source);
                    PlayerPrefs.SetString("urlt", source);
                    PlayerPrefs.SetString(DRAGON_KEY, "Dragon");
                    FB.LogAppEvent(AppEventName.CompletedRegistration);
                    Dragon = false;
                }
            }
    ...
    public void CheckHTMLContent(string content)
    {
        ...
        if (content == "<html><head></head><body></body></html>" && source == webView.Url)
        {
            ...
            if (Loader != null)
                Loader.SetActive(false);
        }else if (Regex.IsMatch(content, "\\b500 Internal Server Error\\b"))
        {
            Debug.LogError("skip error 500");
            webView.Hide();
            if (Game != null)
                Game.SetActive(true);
            if (Loader != null)
