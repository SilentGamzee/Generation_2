...
namespace Assets.Scripts.Main
{
    class MoveCoordinator : MonoBehaviour
    {
        ...
        void UpdateRowColPos(Transform trans, bool IsRow)
        {
            float row_step = MoveSpeed * Time.deltaTime * 0.6f;
            float row_y;
            if(IsRow)
            {
                if (isRowMovingDown)
            ...
            if (Math.Abs(trans_row.position.y - row_y) <= 0.1f)
        ...
        void Update()
        {
            ...
            UpdateRowColPos(trans_row, true);
        }
    }
}
