    ...
    class MoveCoordinator : MonoBehaviour
    {
        ...
        void UpdateBallPos(Transform trans, float step)
        {
            ...
            for (var i = 0; i < trans.childCount; i++)
            {
                ...
                item.position = Vector3.MoveTowards(r_pos, next_point, step);

                if ((Math.Abs(r_pos.x - next_point.x) <= 0.01f) && (Math.Abs(r_pos.y - next_point.y) <= 0.3f))
                {
                    if (Math.Abs(r_pos.x - vorota_pos.x) <= 0.01f)
                        P++;
