...
namespace Assets.Scripts.Main
{
    class MoveCoordinator : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            float row_step = MoveSpeed * param * Time.deltaTime * 0.01f;
            float row_y;
            if (isRowMovingDown)
            {
                row_bottom_y
                trans_row.position = Vector3.MoveTowards(trans_row.position, new Vector3(trans_row.position.x, row_bottom_y, trans_row.position.z), row_step);
            }
            else
            {
                trans_row.position = Vector3.MoveTowards(trans_row.position, new Vector3(trans_row.position.x, row_top_y, trans_row.position.z), row_step);
            }
        }
    }
}
