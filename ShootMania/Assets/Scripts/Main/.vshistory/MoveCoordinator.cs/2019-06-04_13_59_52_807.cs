...
namespace Assets.Scripts.Main
{
    class MoveCoordinator : MonoBehaviour
    {
        ...
        void UpdateRowColPos(Transform trans, float param, bool IsRow)
        {
            ...
            trans_row.position = Vector3.MoveTowards(trans_row.position, new Vector3(trans_row.position.x, row_pos, trans_row.position.z), row_step);
            if (Math.Abs(trans_row.position.y - row_pos) <= 0.1f)
